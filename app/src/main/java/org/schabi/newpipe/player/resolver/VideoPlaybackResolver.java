...
public class VideoPlaybackResolver implements PlaybackResolver {
    ...
    public MediaSource resolve(@NonNull final StreamInfo info) {
        ...
        if (subtitlesStreams != null) {
            final boolean geminiTranslationEnabled = PreferenceManager
                    .getDefaultSharedPreferences(context)
                    .getBoolean(context.getString(
                            R.string.gemini_subtitle_translation_enabled_key), false);
            final String geminiApiKey = GeminiSubtitleSettings.getApiKey(context);
            final String geminiTargetLanguage = PreferenceManager
                    .getDefaultSharedPreferences(context)
                    .getString(context.getString(
                            R.string.gemini_subtitle_target_language_key), "English");
            final boolean geminiShowOriginal = PreferenceManager
                    .getDefaultSharedPreferences(context)
                    .getBoolean(context.getString(
                            R.string.gemini_subtitle_show_original_key), true);
            ...
            for (final SubtitlesStream subtitle : nonTorrentAndUrlStreams) {
                ...
                if (mediaFormat != null) {
                    ...
                    if (geminiTranslationEnabled && !geminiApiKey.isBlank()
                            && geminiTargetLanguage != null
                            && !geminiTargetLanguage.isBlank()
                            && GeminiSubtitleDataSource.isSupported(mediaFormat)) {
                        final MediaItem.SubtitleConfiguration translatedTextMediaItem =
                                new MediaItem.SubtitleConfiguration.Builder(
                                        GeminiSubtitleDataSource.buildUri(
                                                subtitle.getContent(),
                                                info.getServiceId() + ":" + info.getId() + ":"
                                                        + subtitle.getLanguageTag() + ":"
                                                        + subtitle.isAutoGenerated(),
                                                mediaFormat,
                                                subtitle.getLanguageTag(),
                                                geminiTargetLanguage,
                                                geminiShowOriginal))
                                        .setMimeType(MediaFormat.VTT.getMimeType())
                                        .setRoleFlags(textRoleFlag)
                                        .setLanguage(PlayerHelper.captionLanguageOf(
                                                context, subtitle) + " -> "
                                                + geminiTargetLanguage + " (Gemini)")
                                        .build();
                        final MediaSource translatedTextSource = dataSource
                                .getTranslatedSubtitleMediaSourceFactory()
                                .createMediaSource(translatedTextMediaItem, TIME_UNSET);
                        mediaSources.add(translatedTextSource);
                    }
                }
            }
        }
